...
export function GetDOJXNFTContract(params: DOJNFTContractParams) {
  ...
  contract ${params.name}${params.id} is ERC721Burnable, AccessControl {
      string public xnft_name;
      string public xnft_symbol;
      address public omniChainNFTContract;
      bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
  
      event OmniChainNFTContractUpdated(address omniChainContract);
  
      constructor() ERC721(xnft_name, xnft_symbol) {
          xnft_name = "${params.name}";
          xnft_symbol = "${params.symbol}";
      ...
      modifier onlyOmniChainNFTContract() {
          require(msg.sender == omniChainNFTContract, "${params.name}${params.id}: Unauthorized");
          _;
      }
...
export default function DojimaXNftTemplateView({
  displayCode,
  selectedChain,
}: {
  ...
  const Contract_Id = "XNftContract";
  ...
  useEffect(() => {
    ...
    const finalContract = GetDOJXNFTContract(nftOptions);
  ...
  //     symbol
  //   };
  //   const finalContract = GetDOJXNFTContract(erc20Options);
  //   setContract(finalContract);
  //   displayCode(finalContract);
  // }, []);
