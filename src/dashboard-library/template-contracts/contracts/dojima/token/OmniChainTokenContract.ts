export const OmniChainTokenContractTemplate = `// SPDX-License-Identifier: MIT
...
contract OmniChainTokenContract is  Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, AccessControl, IStateReceiver {
    ...
    modifier onlyStateSyncer() {
        require(msg.sender == address(_stateVerifier.stateSyncer()), "OmniChainTokenContract: Caller is not the state syncer");
        _;
    }
    ...
    function initialize(address _xTokenAddress, address _outboundStateSender, address _stateSyncerVerifier) public initializer {
        require(_xTokenAddress != address(0), "OmniChainTokenContract: XToken address cannot be zero");
        require(_outboundStateSender != address(0), "OmniChainTokenContract: OutboundStateSender address cannot be zero");
        require(_stateSyncerVerifier != address(0), "OmniChainTokenContract: Invalid state syncer verifier address");
    ...
    function transferToChain(
        ...
        require(
            ...
            "OmniChainTokenContract: Destination contract address does not match"
        );
    ...
    function onStateReceive(uint256 /* id */, bytes calldata data) external onlyStateSyncer {
        ...
        require(userAddress != address(0), "OmniChainTokenContract: Invalid address");
