...
async function deployContracts(contractsData: SolDojTokenTemplateParams) {
  // let deployedDetails: DeployedDetails = [];

  try {
    const url = `${
      import.meta.env.VITE_APP_MAGIC_DASHBOARD_BACKEND_URL
    }/sol-doj-token/deploy`;
...
export const SolEvmTokenDialog = ({ onClose }: { onClose?: () => void }) => {
  ...
  const { solEvmTokenContractsData, resetSolEvmTokenContractDetails } =
    useContractDetails();
  const {solDojTokenData, resetSolDojTokenDetails} = useSolanaTemplatesDetails();
  ...
  function handleDeploy() {
    ...
    const data: SolDojTokenTemplateParams = solDojTokenData as SolDojTokenTemplateParams;
  ...
  const renderDetailsForChain = (chain: AvailableChains) => {
    const contract = solEvmTokenContractsData.solEvmTokenContracts[tab - 1];
    if (!contract) {
      return null;
    }
      ...
      {isDeployed ? (
        ...
        >
          ...
          <div className="HelpDialog__header">
            {solEvmTokenContractsData.solEvmTokenContracts.map((item, i) => {
              const index = i + 1;
          ...
          <div className="flex justify-between mt-2 items-center">
            ...
            <Button
              ...
              onClick={
                tab === solEvmTokenContractsData.solEvmTokenContracts.length
                  ? handleDeploy
                  : () => {
                      setTab(tab + 1);
                    }
              ...
              disabled={
                (tab === solEvmTokenContractsData.solEvmTokenContracts.length &&
                  !(
                    userDetails.chains.length === solEvmTokenContractsData.solEvmTokenContracts.length
                  )) 
                //   ||
