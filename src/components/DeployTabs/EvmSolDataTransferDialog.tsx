...
export type DeployedDetailsObject = {
  chain: AvailableChains;
  details: DojimaDeployedData | SolanaDeployedData | string;
}

export type DeployedDetails = Array<DeployedDetailsObject>;
...
async function addDeployedDetailsToDb(
  email: string,
  projectName: string,
  contractDetails: DeployedDetails,
) {
...
async function deployContracts(contractsData: DojSolDataTransferTemplateParams) {
  // let deployedDetails: DeployedDetails = [];

  try {
    const url = `${
      import.meta.env.VITE_APP_MAGIC_DASHBOARD_BACKEND_URL
    }/doj-sol-data-transfer/deploy`;
...
export const EvmSolDataTransferDialog = ({ onClose }: { onClose?: () => void }) => {
  ...
  const { evmSolDataTransferContractsData, resetEvmSolDataTransferContractDetails } =
    useContractDetails();
  const {dojSolDataTransferData, resetDojSolDataTransferDetails} = useSolanaTemplatesDetails();
    ...
    switch (chain) {
      ...
      case "BSC":
        ...
        case "sol":
          case "solana":
          case "SOL":
            return import.meta.env.VITE_APP_SOL_DEVNET_EXPLORER_URL;
    }
  }
  ...
  function handleDeploy() {
    ...
    const data: DojSolDataTransferTemplateParams = dojSolDataTransferData as DojSolDataTransferTemplateParams;
  ...
  const renderDetailsForChain = (chain: AvailableChains) => {
    const contract = evmSolDataTransferContractsData.evmSolDataTransferContracts[tab - 1];
    if (!contract) {
      return null;
    }
      ...
      {isDeployed ? (
        ...
        >
          <div className="px-7 py-3">
            ...
            <div className="grid-cols-[0.6fr_2fr_0.5fr] body16 items-center grid border-b border-outline mt-1 pb-2 last:border-none">
              {deployedDetails.map((item: DeployedDetailsObject) => (
                <>
                  ...
                  <p className="text-sm mt-4 text-black">
                    <a
                      ...
                      // }`}
                      href={`${
                        item.chain.toLowerCase() === "solana"
                          ? `${getChainExplorerUrl(
                              item.chain
                            )}${(item.details as SolanaDeployedData).programId}?cluster=devnet`
                          : `${getChainExplorerUrl(
                              item.chain
                            )}address/${(item.details as DojimaDeployedData).contractAddress}`
                      }`}
                    ...
                    >
                      {item.chain === 'solana' ? (item.details as SolanaDeployedData).programId : (item.details as DojimaDeployedData).contractAddress}
                    </a>
                  </p>
                  <div
                    className="cursor-pointer mt-4"
                    onClick={() =>
                      copyTextToSystemClipboard(item.chain === 'solana' ? (item.details as SolanaDeployedData).programId : (item.details as DojimaDeployedData).contractAddress)
                    }
                  >
                    <CopyIcon width="15" />
                  </div>
        ...
        >
          ...
          <div className="HelpDialog__header">
            {evmSolDataTransferContractsData.evmSolDataTransferContracts.map((item, i) => {
              const index = i + 1;
          ...
          <div className="flex justify-between mt-2 items-center">
            ...
            <Button
              ...
              onClick={
                tab === evmSolDataTransferContractsData.evmSolDataTransferContracts.length
                  ? handleDeploy
                  : () => {
                      setTab(tab + 1);
                    }
              ...
              disabled={
                (tab === evmSolDataTransferContractsData.evmSolDataTransferContracts.length &&
                  !(
                    userDetails.chains.length === evmSolDataTransferContractsData.evmSolDataTransferContracts.length
                  )) 
                //   ||
