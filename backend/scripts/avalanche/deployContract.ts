...
async function compile(contract: string, contractName: string) {
  ...
  writeFileSync(`${process.cwd()}/contracts/${contractName}-avax.sol`, code);
...
async function deploy(
  ...
  const provider = new ethers.JsonRpcProvider(
    process.env.VITE_APP_AVAX_C_TESTNET_API_URL as string,
  );
  ...
  try {
    const jsonData = JSON.parse(
      readFileSync(
        `${process.cwd()}/artifacts/contracts/${contractName}-avax.sol/${contractName}.json`,
      ).toString(),
    );
...
export async function deployAVAXContractHandler(
